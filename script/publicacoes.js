function publicacoes(){
  ...
  animar('sample1-publicacoes', 'aa1-nav', 'Sample book', ()=>{configurarPdf(1)}, ['texto-publicacoes', 'clicavel'])
  ...
  animar('sample2-publicacoes', 'aa2-nav', 'Sample book', ()=>{configurarPdf(2)}, ['texto-publicacoes', 'clicavel'])
...
function configurarPdf(numAA) {
  let arquivo = `img/publi/aa${numAA}_${ptBr? 'pt' : 'es'}.pdf`
  console.log(arquivo);
  ...
  document.getElementById('botao-voltar').addEventListener('click', (e) => {
      if(estado.pdf == null || estado.numPagina == 1)
        return;
      estado.numPagina -= 1;
  ...
  document.getElementById('num-pagina').addEventListener('keypress', (e) => {
      ...
      if(code == 13) {
          var desiredPage =
          document.getElementById('num-pagina').valueAsNumber;
